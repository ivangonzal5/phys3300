...
vector<string> generateInventory(int spaces, int maxGen)
{
...
vector<string> recoleccion(vector<string> inventory)
{
	...
	{
		randomInventory = generateInventory(5, 5);
	...
	while (recoleccionMenuOpen) {
		...
		cout << "= = RECOLECCION = =" << "\n" << endl;
		//Creamos el ciclo que itera sobre el inventario para imprimirlo (mostrarlo en pantalla).
		for (int counter = 0; counter < randomInventory.size(); counter++) {

			cout << counter + 1 << ". " << randomInventory[counter] << endl;

		}

		//Creamos la opción EXIT.
		cout << randomInventory.size() + 1 << ". " << "EXIT" << endl;
		cout << "===============================================================" << endl;

		//OPCION PARA HACER REFILL A LA LISTA DE RECOLECCION
		cout << randomInventory.size() + 2 << ". " << "REFILL LIST (DEV ONLY)" << endl;
		cout << "===============================================================" << endl;
		...
		if (recoleccionMenuOption <= randomInventory.size()) {
			...
			else
			{
				...
				if (randomInventory[recoleccionMenuOption - 1] == "Crystallized Azure Dragon's Tears" || randomInventory[recoleccionMenuOption - 1] == "Crystallized Great Feline's Ash" || randomInventory[recoleccionMenuOption - 1] == "Crystallized Exiled Heretic's Bone")
				{
					crystalItem = true;
				}
				...
				else
				{
					...
					inventory[inventory.size() - naSpaces] = randomInventory[recoleccionMenuOption - 1];
					randomInventory.erase(randomInventory.begin() + recoleccionMenuOption - 1);
		...
		else  if (recoleccionMenuOption == randomInventory.size() + 1) 
		{
			recoleccionMenuOpen = false;
		}
		//CIERRA EL MENU Y LA PROXIMA VEZ HARA REFILL DE ITEMS.
		else if (recoleccionMenuOption == randomInventory.size() + 2)
		{
			RecoleccionGenerated = false;
			recoleccionMenuOpen = false;
		}
		//RETORNA EL NUEVO INVENTARIO
		return inventory;
...
int main()
{
	...
	while (menuOpen) {
		...
		case 1:
			inventory = generateInventory(20, 7);
			break;
		...
		case 4:
			...
			inventory = recoleccion(inventory);
			break;
